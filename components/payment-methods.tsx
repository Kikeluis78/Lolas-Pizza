...
export function PaymentMethods() {
  ...
  const validateContactData = () => {
    ...
    if (serviceType !== "restaurant") return true
  ...
  const handlePaymentMethodChange = (value: string) => {
    if (!validateContactData()) {
      Swal.fire({
        ...
        text: "Por favor ingresa tu nombre y teléfono antes de seleccionar el método de pago.",
  ...
  const handleFinishOrder = () => {
    ...
    // Validar datos de contacto si es recoger en restaurante
    const serviceType = localStorage.getItem("serviceType")
    if (serviceType === "restaurant") {
      ...
      if (userAccount) {
        const user = JSON.parse(userAccount)
        hasContact = Boolean(user.name && user.phone)
      } else if (guestContact) {
        const contact = JSON.parse(guestContact)
        hasContact = Boolean(contact.name && contact.phone)
      }
      ...
      if (!hasContact) {
        Swal.fire({
          ...
          text: "Por favor completa tu nombre y teléfono en la sección de Datos de Contacto",
        })
        return
      }
    }
      ...
      <CardContent className="space-y-6">
        ...
        <div className="pt-4 border-t space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold">${mounted ? getTotal() : 0}</span>
          </div>

          {appliedCoupon && (
            <div className="flex justify-between text-sm text-green-600 dark:text-green-400">
              <span>Descuento ({appliedCoupon.discount}%):</span>
              <span className="font-semibold">-${mounted ? calculateDiscount().toFixed(2) : '0.00'}</span>
            </div>
          )}
          ...
          <div className="flex justify-between text-xl font-bold pt-2 border-t">
            ...
            <span className="text-primary">${mounted ? getFinalTotal().toFixed(2) : '0.00'}</span>
          </div>
